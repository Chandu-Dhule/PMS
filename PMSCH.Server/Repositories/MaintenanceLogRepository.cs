    ...
    public class MaintenanceLogRepository
    {
        ...
        public void Add(MaintenanceLog log)
        {
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
                ...
                {
                    // Generate new LogID
                    string maxLogIdQuery = "SELECT ISNULL(MAX(LogID), 0) + 1 FROM MaintenanceLogs";
                    SqlCommand maxCmd = new SqlCommand(maxLogIdQuery, conn, transaction);
                    log.LogID = (int)maxCmd.ExecuteScalar();

                    // Insert into MaintenanceLogs (without Status)
                    string insertLogQuery = @"INSERT INTO MaintenanceLogs 
                (LogID, MachineID, MaintenanceDate, Description, OperatorName, NextDueDate) 
                VALUES 
                (@LogID, @MachineID, @MaintenanceDate, @Description, @OperatorName, @NextDueDate)";
                    SqlCommand logCmd = new SqlCommand(insertLogQuery, conn, transaction);
                ...
                AND MetricID = (
                    ...
                    SqlCommand metricCmd = new SqlCommand(updateMetricQuery, conn, transaction);
                    ...
                    string updateMachineStatusQuery = @"UPDATE Machines 
                SET Status = @Status 
                WHERE MachineID = @MachineID";
                    SqlCommand statusCmd = new SqlCommand(updateMachineStatusQuery, conn, transaction);
                    statusCmd.Parameters.AddWithValue("@Status", log.Status);
                    statusCmd.Parameters.AddWithValue("@MachineID", log.MachineID);
