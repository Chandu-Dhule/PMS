...
namespace PMSCH.Server.Repositories
{
    public class TechnicianMachineAssignmentRepository
    {
        ...
        public List<int> GetAssignedMachines(int userId)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT MachineId FROM TechnicianMachineAssignments WHERE UserId = @UserId", conn);
        ...
        public void RemoveAssignment(int userId, int machineId)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("DELETE FROM TechnicianMachineAssignments WHERE UserId = @UserId AND MachineId = @MachineId", conn);
                cmd.Parameters.AddWithValue("@UserId", userId);
